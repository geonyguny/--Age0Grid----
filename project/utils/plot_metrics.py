...
def main():
    ...
    print(f"[report] Top{args.topn} ??? {out_csv}")
    ...
    if {"tag","EW"}.issubset(df.columns):
        ...
        if not sub.empty:
            ...
            plt.savefig(out_png, dpi=160, bbox_inches="tight")
            plt.close()
            print(f"[report] 諛뺤뒪?뚮’ ??? {out_png}")

    # 3) cap-entropy 寃⑹옄 ?됯퇏 EW ?덊듃留??좎궗 ?뚯씠釉?csv)
    ...
    if {"cap","entropy","EW"}.issubset(have_cols):
        ...
        print(f"[report] 寃⑹옄 ?됯퇏 EW ??? {grid_csv}")
    ...
    if {"method","EW","ES95","Ruin"}.issubset(have_cols):
        ...
        print(f"[report] 諛⑸쾿濡??붿빟 ??? {summ_csv}")
