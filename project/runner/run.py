...
def _bias_meta_from_args(args: Any) -> Dict[str, Any]:
    try:
        return {
            "bias_on": str(getattr(args, "bias_on", "off")).lower()
            in ("on", "true", "1", "yes", "y"),
            "loss_aversion": float(
                getattr(args, "bias_loss_aversion", 0.0) or 0.0
            ),
            "prob_gamma": float(
                getattr(args, "bias_prob_gamma", 1.0) or 1.0
            ),
            "myopia": float(getattr(args, "bias_myopia", 0.0) or 0.0),
            "w_floor": float(getattr(args, "bias_w_floor", 0.0) or 0.0),
            "w_cap_shock": float(
                getattr(args, "bias_w_cap_shock", 0.0) or 0.0
            ),
        }
    except Exception:
        return {}
...
def _inject_market_meta(cfg: SimConfig, args, out_dict: Dict[str, Any]) -> None:
    ...
    market_meta_from_cfg = (
        getattr(cfg, "meta", {}).get("market", {})
        if getattr(cfg, "meta", None)
        else {}
    )
    ...
    meta_market = (
        dict(market_meta_from_cfg)
        if isinstance(market_meta_from_cfg, dict)
        else {}
    )
    meta_market.update(
        {
...
def _wire_market_data(cfg: SimConfig, args) -> None:
    ...
    if ret_fx is None or (
        hasattr(ret_fx, "size") and ret_fx.size == 0
    ):
    ...
    if ret_us_l is None:
        ...
        if ret_fx is not None:
            ...
            ret_us_hedged = (
                np.asarray(ret_us_l, dtype=float) - h_fx * fx_cost_m
            )

        a_kr, a_us, a_au = _parse_alpha_mix(args)
        kr = (
            np.asarray(ret_kr, dtype=float)
            if ret_kr is not None
            else 0.0
        )
        us = np.asarray(ret_us_hedged, dtype=float)
        au = (
            np.asarray(ret_au, dtype=float)
            if ret_au is not None
            else 0.0
        )

        lens = [
            x.shape[0] for x in [kr, us, au] if isinstance(x, np.ndarray)
        ]
    ...
    setattr(
        ...
        "data_rf_series",
        rf_real
        if _onoff(getattr(args, "use_real_rf", "on")) == "on"
        else rf_nom,
    )
...
# --------------------------
# Annuity setup helper (공통)
# --------------------------
def _setup_annuity_if_enabled(cfg: SimConfig, args: Any) -> bool:
    """
    annuity overlay 설정 공통 헬퍼.

    정책은 annuity_wiring.setup_annuity_overlay 에 위임:
      - ann_on 이 'on'/'off' 이거나, 'auto'/None 인 경우
      - ann_alpha > 0 여부에 따라 ON/OFF 결정
      - 성공 시 cfg 에 ann_on='on', y_ann, ann_P, ann_a_factor, W0(연금 매입 후) 주입
      - 실패/미적용 시 내부에서 안전하게 OFF 상태로 정리

    여기서는 meta 를 cfg.ann_meta / cfg.ann_W_after 에 기록하고
    실제 적용 여부만 bool 로 반환한다.
    """
    try:
        meta = setup_annuity_overlay(cfg, args)
    except Exception:
        # 치명적 오류 시 연금 비활성 처리
        setattr(cfg, "ann_meta", None)
        return False
    ...
    # meta 기록
    try:
        setattr(cfg, "ann_meta", dict(meta))
    except Exception:
        setattr(cfg, "ann_meta", None)

    # 편의상 W_after 를 별도 필드로 노출 (없으면 cfg.W0 그대로 사용)
    try:
        W0_after = float(meta.get("W0_after", getattr(cfg, "W0", 0.0)))  # type: ignore[arg-type]
        setattr(cfg, "ann_W_after", W0_after)
    except Exception:
        # 실패 시에도 최소한 필드는 존재하게
        if not hasattr(cfg, "ann_W_after"):
            try:
                setattr(cfg, "ann_W_after", getattr(cfg, "W0", 0.0))
            except Exception:
                pass
...
def _to_actor(policy_like: Any) -> Callable[[Any], tuple[float, float]]:
    ...
    def _actor(state: Any):
        ...
        if hasattr(policy_like, "act"):
            ...
            raise RuntimeError(
                "actor must return (q, w) or dict with keys 'q','w'"
            )
...
def _as_ndarray_state(
    raw_obs: Union[Dict[str, Any], _np.ndarray, None], env: Any
) -> _np.ndarray:
    ...
    if isinstance(raw_obs, _np.ndarray):
        ...
        return _np.asarray(
            [float(arr[0]) if arr.size >= 1 else 0.0, W_now], dtype=float
        )
    return _np.asarray([0.0, float(getattr(env, "W", 0.0))], dtype=float)


def _rollout_terminal_wealths(
    cfg: SimConfig, actor: Callable[[Any], tuple[float, float]], n_paths: int
) -> List[float]:
...
def _compute_metrics_from_wt(
    wt, es_mode: str, alpha: float, F_target: float
) -> Dict[str, Any]:
    ...
    if es_mode == "wealth":
        ...
        es95 = _cvar_loss_from_wealth(
            wt, F_target=F_target, alpha=alpha
        )
    return {"EW": ew, "mean_WT": ew, "ES95": es95, "Ruin": ruin}


def _diversify_when_degenerate(
    xs: List[float], want_n: int | None = None
) -> List[float]:
...
def _ref_cstar_for_eval(args: Any, env: Any, obs: Any) -> float:
    ...
    if mode == "fixed":
        cstar_m = _safe_float(
            getattr(args, "cstar_m", 0.04 / 12), 0.04 / 12
        )
        if cstar_m > 0.2:
    ...
    try:
        ...
        if isinstance(monthly, dict):
            p_m = float(
                monthly.get("p_m", 0.04 / 12) or 0.04 / 12
            )
        else:
            p_m = (
                float(getattr(args, "cstar_m", 0.04) or 0.04) / 12.0
            )
    except Exception:
        p_m = 0.04 / 12
...
def _standard_evaluate(
    cfg: SimConfig, actor_like: Any, args: Any
    ...
    if isinstance(ret, dict):
        ...
        metrics = {
            "note": "unexpected evaluate return type",
            "type": str(type(ret)),
        }
    ...
    want_paths = (
        str(getattr(args, "print_mode", "full")).lower() == "full"
    ) and (not getattr(args, "no_paths", False))
    wt = (extras or {}).get("eval_WT", None)

    if (
        (not isinstance(wt, list))
        or len(wt) == 0
        or _looks_degenerate_wt(wt)
    ):
        if want_paths:
            n_paths = getattr(args, "n_paths", None)
            n_paths = (
                int(n_paths)
                if (n_paths is not None and int(n_paths) > 0)
                else int(getattr(cfg, "n_paths_eval", 0) or 500)
            )
            try:
                WTs = _rollout_terminal_wealths(
                    cfg, _to_actor(actor_like), n_paths
                )
                extras = extras or {}
                ...
                extras.setdefault(
                    "eval_WT_note",
                    f"local rollout failed: {type(_e).__name__}",
                )
    ...
    if isinstance(wt, list) and getattr(args, "n_paths", None):
        ...
        if n_req > 0 and len(wt) != n_req:
            wt = (
                wt[:n_req]
                if len(wt) > n_req
                else (wt + [wt[-1]] * (n_req - len(wt)))
            )
            extras["eval_WT"] = wt

    if isinstance(wt, list) and _looks_degenerate_wt(wt):
        extras["eval_WT"] = _diversify_when_degenerate(
            wt,
            want_n=int(
                getattr(args, "n_paths", len(wt)) or len(wt)
            ),
        )
        wt = extras["eval_WT"]
    ...
    if (
        isinstance(wt, list)
        and len(wt) > 0
        and (not _looks_degenerate_wt(wt))
    ):
        recalc = _compute_metrics_from_wt(
            wt, es_mode=es_mode, alpha=alpha, F_target=F_target
        )
        metrics.update(recalc)
        metrics["es95_source"] = (
            f"computed_from_eval_WT_{es_mode}"
        )
    ...
    try:
        ...
        metrics.setdefault(
            "ret_mean",
            float(_np.nanmean(_mixed)) if _mixed is not None else None,
        )
        metrics.setdefault(
            "rf_mean",
            float(_np.nanmean(_rf)) if _rf is not None else None,
        )
        if _a_mix is not None:
            metrics.setdefault(
                "alpha_mix_used", tuple(map(float, _a_mix))
            )
        if _hfx is not None:
            metrics.setdefault("h_FX_used", float(_hfx))
        fx_cost_ann = getattr(cfg, "fx_hedge_cost_annual", None)
        if fx_cost_ann is not None:
            metrics.setdefault(
                "fx_hedge_cost_annual", float(fx_cost_ann)
            )
    except Exception:
        pass
...
def _save_metrics_files(out_dir: Path, metrics: Dict[str, Any], args: Any) -> None:
    ...
    row = {
        ...
        "mean_WT": float(
            metrics.get("mean_WT", ew if ew is not None else 0.0)
        ) if (metrics.get("mean_WT", None) is not None or ew is not None) else None,
        ...
        "ann_alpha":     _safe_float(metrics.get("ann_alpha")),
        "y_ann":         _safe_float(metrics.get("y_ann")),
        "ann_a_factor":  _safe_float(metrics.get("ann_a_factor")),
        "P":             _safe_float(metrics.get("P")),
        "W_after_ann":   _safe_float(metrics.get("W_after_ann")),
...
def _promote_diagnostics_to_metrics(
    ...
    if trainer is not None:
        for cand in [
            ...
            getattr(getattr(trainer, "stats", None) or {}, "get", lambda *_: None)(
                "la_sf_mean"
            ),
            getattr(trainer, "diagnostics", {}).get("la_sf_mean")
    ...
    if args is not None:
        ...
        _maybe_set(
            "bias_loss_aversion",
            _safe_float(getattr(args, "bias_loss_aversion", None)),
        )
...
def run_once(args) -> Dict[str, Any]:
    ...
    with quiet_ctx:
        ...
        # annuity 세팅 (공통 헬퍼 사용: 내부에서 probe env로 y_ann 등 주입)
        t2 = time.perf_counter()
        ann_enabled = _setup_annuity_if_enabled(cfg, args)
    ...
    # annuity 수치들을 metrics에 확실히 기록
    if isinstance(m, dict):
        # ann_alpha: cfg > args > 0.0
        try:
            ann_alpha_val = float(
                getattr(cfg, "ann_alpha", getattr(args, "ann_alpha", 0.0))
                or 0.0
            )
        except Exception:
            ann_alpha_val = 0.0

        # ann_on: cfg 우선, 없으면 ann_alpha에 따라 auto on/off,
        # args에서 명시적 off가 들어오면 off로 강제
        ann_on_raw = getattr(cfg, "ann_on", None)
        if not ann_on_raw:
            flag_raw = getattr(args, "ann_on", None)
            if flag_raw is None:
                ann_on_raw = "on" if ann_alpha_val > 0.0 else "off"
            else:
                ann_on_raw = _onoff(flag_raw, default="off")
        ann_on_norm = _onoff(ann_on_raw, default="off")
        if ann_alpha_val <= 0.0:
            ann_on_norm = "off"

        # cfg / meta 에서 가져온 연금 관련 값
        y_ann_cfg = _safe_float(getattr(cfg, "y_ann", None), None)
        a_fac_cfg = _safe_float(getattr(cfg, "ann_a_factor", None), None)
        P_cfg = _safe_float(getattr(cfg, "ann_P", None), None)
        # ann_W_after 가 있으면 사용, 없으면 W0 그대로
        W_after_cfg = _safe_float(
            getattr(cfg, "ann_W_after", getattr(cfg, "W0", None)), None
        )

        # metrics에 주입
        m.setdefault("ann_on", ann_on_norm)
        m.setdefault("ann_alpha", ann_alpha_val)

        if y_ann_cfg is not None:
            m["y_ann"] = y_ann_cfg
        if a_fac_cfg is not None:
            m["ann_a_factor"] = a_fac_cfg
            m["a_factor"] = a_fac_cfg
        if P_cfg is not None:
            m["P"] = P_cfg
        if W_after_cfg is not None:
            m["W_after_ann"] = W_after_cfg
    ...
    n_paths_total = len(extras.get("eval_WT", [])) or (
        (getattr(cfg, "n_paths_eval", getattr(cfg, "n_paths", 0)) or 0)
        * max(1, len(getattr(cfg, "seeds", [])))
    )
    ...
    out = dict(
        ...
        fee_annual=getattr(
            cfg, "phi_adval", getattr(cfg, "fee_annual", None)
        ),
    ...
    meta = {
        "tag": getattr(args, "tag", None) or getattr(cfg, "tag", None),
        "method": out["method"],
        "asset": getattr(cfg, "asset", None),
    ...
    try:
        # ★ tag는 args.tag → 없으면 cfg.tag 사용
        tag = getattr(args, "tag", None) or getattr(cfg, "tag", None)
        if tag:
            _save_metrics_files(
                Path(args.outputs) / tag, out.get("metrics", {}) or {}, args
            )
    except Exception:
        pass
...
def _build_env_factory_from_args(args, cfg: SimConfig):
    ...
    fee_annual = (
        float(args.phi_adval)
        if (getattr(args, "phi_adval", None) not in (None, 0.0))
        else float(args.fee_annual)
    )
    base_kwargs = dict(
...
def _deterministic_policy_step(tr, obs, device):
    ...
    a_q = (
        dist_q.concentration1
        / (dist_q.concentration1 + dist_q.concentration0)
    ).squeeze(-1)
    a_w = (
        dist_w.concentration1
        / (dist_w.concentration1 + dist_w.concentration0)
    ).squeeze(-1)
...
def _evaluate_collect_WT(
    tr, env_factory, n_episodes: int, eval_seed_jitter: bool = False, args: Any = None
) -> Dict[str, Any]:
    WT = []
    returns = []
    ...
    cstar_mode = str(
        getattr(args, "cstar_mode", "annuity") or "annuity"
    ).lower()
    ...
    def _pct(a: _np.ndarray, q: float):
        try:
            return float(_np.nanpercentile(a, q))
        except Exception:
            return None
    ...
    for ep in range(int(n_episodes)):
        ...
        info = {}
        if args is not None and _onoff(
            getattr(args, "quiet", "on")
        ) != "on":
            try:
                print(
                    f"[BIAS] on={getattr(args,'bias_on','off')} "
        ...
        while not done:
            # 정책(기본) + 행동편향 래핑
            base_act = _deterministic_policy_step(
                tr, obs, tr.device
            )
            q0, w0 = float(base_act.get("q", 0.0)), float(
                base_act.get("w", 0.0)
            )
            q_b, w_b = q0, w0
            try:
            ...
            la_sf_fracs.append(
                max(c_star_amt - c_val, 0.0) / max(c_star_amt, eps)
            )
        ...
        if la_sf_mean_seen is None:
            for cand in [
                ...
                getattr(getattr(tr, "diagnostics", None) or {}, "get", lambda *_: None)(
                    "la_sf_mean"
                ),
            ]:
                if cand is not None:
    ...
    la_sf_rate_calc = float(
        _np.mean(_np.asarray(la_sf_fracs) > 0.0)
    ) if len(la_sf_fracs) else 0.0

    WT_arr = _np.asarray(WT, dtype=float)
    if WT_arr.size:
        # log10 평균(고갈 수치 안정화)
        log10_WT_mean = float(
            _np.mean(_np.log10(_np.clip(WT_arr, 1e-300, None)))
        )
        WT_p5, WT_p50, WT_p95 = (
            _pct(WT_arr, 5),
            _pct(WT_arr, 50),
    ...
    out = {
        ...
        "eval_return_mean": float(_np.mean(returns))
        if len(returns)
        else 0.0,
        "eval_return_std": float(_np.std(returns))
        if len(returns)
        else 0.0,
        "eval_seed_mode": "jitter" if eval_seed_jitter else "fixed",
        "eval_seed_base": int(base_seed),
        "la_sf_mean": float(
            la_sf_mean_calc if la_sf_mean_seen is None else la_sf_mean_seen
        ),
        ...
        "mean_cstar_amt": float(_np.mean(cstar_list))
        if cstar_list
        else None,
        "mean_consumption_amt": float(_np.mean(cons_list))
        if cons_list
        else None,
    }
    return out
...
def run_rl(args):
    ...
    with quiet_ctx:
        ...
        try:
            single_seed = (
                int(getattr(args, "seed", None))
                if getattr(args, "seed", None) is not None
                else int(cfg.seeds[0])
            )
        except Exception:
            single_seed = 0
        ...
        cfg_rl = RLConfig(
            ...
            ent_coef=float(
                getattr(args, "entropy_coef", 0.005) or 0.005
            ),
            vf_coef=float(
                getattr(args, "value_coef", 0.5) or 0.5
            ),
            lr=float(getattr(args, "lr", 3e-4) or 3e-4),
            max_grad_norm=float(
                getattr(args, "max_grad_norm", 0.5) or 0.5
            ),
            max_steps=max_steps,
            rollout_len=int(
                getattr(args, "rl_steps_per_epoch", 512) or 512
            ),
            batch_size=int(
                getattr(args, "rl_steps_per_epoch", 512) or 512
            ),
            ...
            log_dir=os.path.join(
                os.path.abspath(args.outputs), "_logs"
            ),
            tag=str(
                getattr(args, "tag", "rl_run") or "rl_run"
            ),
            device="auto",
        ...
        # [FIX 2026-07] 학습 후반부 정책 붕괴(발산)로 마지막 체크포인트가 최고 성능
        # 시점보다 나쁠 수 있음을 확인했다. train() 내부에서 주기적으로 평가하며
        # 저장해 둔 best.pt가 있으면 그것을 최종 평가에 사용한다(없으면 무해하게
        # 마지막 상태 그대로 진행).
        try:
            best_ckpt_path = os.path.join(trainer.run_dir, "best.pt")
            if os.path.exists(best_ckpt_path):
                trainer.load(best_ckpt_path)
                if str(getattr(args, "verbose", "off")).lower() == "on":
                    print(f"[RL] best.pt 로드 완료: {best_ckpt_path}")
        except Exception as _e:
            if str(getattr(args, "verbose", "off")).lower() == "on":
                print(f"[RL][WARN] best.pt 로드 실패, 마지막 가중치로 진행: {_e!r}")

        t4 = time.perf_counter()
        n_eval = int(
            getattr(args, "rl_n_paths_eval", 0) or 0
        ) or 500
        extras_dict = _evaluate_collect_WT(
            ...
            n_eval,
            eval_seed_jitter=(
                str(getattr(args, "eval_seed_jitter", "off")).lower()
                == "on"
            ),
            args=args,
        )
        debug_eval_mean = extras_dict.pop(
            "eval_return_mean", None
        )
        debug_eval_std = extras_dict.pop(
            "eval_return_std", None
        )
        ...
        metrics_dict: Dict[str, Any] = _compute_metrics_from_wt(
            wt, es_mode=es_mode, alpha=alpha, F_target=F_target
        )
        metrics_dict["es95_source"] = (
            f"computed_from_eval_WT_{es_mode}"
        )
        ...
        try:
            ...
            _hfx = getattr(cfg, "h_FX", None)
            metrics_dict.setdefault(
                "market_len_ret", _len1d(_mixed)
            )
            metrics_dict.setdefault("market_len_rf", _len1d(_rf))
            metrics_dict.setdefault(
                "market_len_dates", _len1d(_dates)
            )
            metrics_dict.setdefault(
                "ret_mean",
                float(_np.nanmean(_mixed))
                if _mixed is not None
                else None,
            )
            metrics_dict.setdefault(
                "rf_mean",
                float(_np.nanmean(_rf))
                if _rf is not None
                else None,
            )
            if _a_mix is not None:
        ...
        _promote_diagnostics_to_metrics(
            metrics_dict, extras=extras_dict, trainer=trainer, args=args
        )

        # annuity 관련 메타를 RL 메트릭에도 명시적으로 반영
        try:
            # ann_alpha: cfg > args > 0.0
            try:
                ann_alpha_val = float(
            ...
            metrics_dict.setdefault("ann_on", ann_on_norm)
            metrics_dict.setdefault("ann_alpha", ann_alpha_val)

            y_ann = _safe_float(getattr(cfg, "y_ann", None), None)
            a_fac = _safe_float(getattr(cfg, "ann_a_factor", None), None)
            P_val = _safe_float(getattr(cfg, "ann_P", None), None)
            W_after = _safe_float(
                getattr(cfg, "ann_W_after", getattr(cfg, "W0", None)),
                None,
            )

            if y_ann is not None:
                metrics_dict.setdefault("y_ann", y_ann)
            if a_fac is not None:
                metrics_dict.setdefault("ann_a_factor", a_fac)
                metrics_dict.setdefault("a_factor", a_fac)
            if P_val is not None:
                metrics_dict.setdefault("P", P_val)
            if W_after is not None:
                metrics_dict.setdefault("W_after_ann", W_after)
        except Exception:
            pass
        ...
        out = dict(
            ...
            | {
                ...
                "teacher_eps0": getattr(
                    args, "teacher_eps0", None
                ),
                "teacher_decay": getattr(
                    args, "teacher_decay", None
                ),
                "survive_bonus": getattr(
                    args, "survive_bonus", None
                ),
        ...
        try:
            ...
            if debug_eval_mean is not None:
                out["meta"]["eval_return_mean"] = float(
                    debug_eval_mean
                )
            if debug_eval_std is not None:
                out["meta"]["eval_return_std"] = float(
                    debug_eval_std
                )
            out["meta"].update(
                {
                    "market_len_ret": metrics_dict.get(
                        "market_len_ret"
                    ),
                    "market_len_rf": metrics_dict.get(
                        "market_len_rf"
                    ),
                    "market_len_dates": metrics_dict.get(
                        "market_len_dates"
                    ),
                    "ret_mean": metrics_dict.get("ret_mean"),
                    "rf_mean": metrics_dict.get("rf_mean"),
                    "alpha_mix_used": metrics_dict.get(
                        "alpha_mix_used"
                    ),
                    "h_FX_used": metrics_dict.get("h_FX_used"),
                    "fx_hedge_cost_annual": metrics_dict.get(
                        "fx_hedge_cost_annual"
                    ),
                }
            )
        except Exception:
        ...
        if _parse_bh is not None and bh_spec is not None:
            try:
                out.setdefault("meta", {})
                out["meta"]["behavioral"] = _bh_describe(
                    bh_spec
                )
            except Exception:
                pass
        ...
        metrics_csv = os.path.join(
            args.outputs, "_logs", "metrics.csv"
        )
        meta = {
            "tag": getattr(args, "tag", None) or getattr(cfg, "tag", None),
            "method": "rl",
        ...
        try:
            ...
            save_metrics_autocsv(
                out.get("metrics", {}), cfg, outputs=args.outputs
            )
        except Exception:
            pass

        try:
            # ★ 여기서도 tag fallback: args.tag → cfg.tag
            tag = getattr(args, "tag", None) or getattr(cfg, "tag", None)
            if tag:
                _save_metrics_files(
                    Path(args.outputs) / tag, metrics_dict, args
                )
        except Exception:
            pass
        ...
        if _onoff(getattr(args, "autosave", "off")) == "on":
            do_autosave(
                out.get("metrics") or {}, cfg, args, out
            )
