...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--value_clip", type=float, default=0.0,
                   help="크리틱 안정화용 PPO 스타일 value clipping(0=미사용). "
                        "loss_v가 학습 후반 불안정하게 튈 때 시도해볼 값(예: 50~200).")
    p.add_argument("--entropy_clip", type=float, default=0.0,
    ...
    p.add_argument("--regret_c_ref_base", choices=["W", "W0"], default="W",
                   help="기준소비(c_ref)의 기준자산. W(기본)=현재자산 비례(기존 동작), "
                        "W0=은퇴시점 초기자산 기준의 고정 목표소비(실질 절대액). W0로 주면 "
                        "자산 감소로 소비가 목표 아래로 떨어질 때 손실회피/후회 페널티가 "
                        "실제로 발동한다(전망이론의 고정 참조점, 논문 식32 c* 해석).")
