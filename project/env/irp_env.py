...
class IRPEnvAdapter:
    ...
    def step(self, action: Dict[str, float]):
        ...
        # [FIX 2026-07] 기존엔 항상 원본(왜곡 안 된) CRRA 효용을 직접 재계산했는데,
        # 이러면 RetirementEnv.step()이 이미 계산해 둔 손실회피/습관형성/후회회피
        # (BehavioralSpec, --bh_on) 왜곡 효용이 RL 학습 보상에 전혀 반영되지 않는
        # 문제가 있었다. --bh_on을 켜도 RL 정책이 그 편향을 전혀 "느끼지" 못했던
        # 근본 원인이다. underlying env가 info에 담아 주는 u_eff(왜곡 반영된 효용)가
        # 있으면 그것을 우선 사용하고, 없으면(구버전 env 등) 기존처럼 원본 소비로부터
        # 직접 재계산하는 것으로 폴백한다.
        u_eff_from_info = None
        if isinstance(info, dict) and "u_eff" in info:
            try:
                u_eff_from_info = float(info["u_eff"])
            except Exception:
                u_eff_from_info = None

        if u_eff_from_info is not None:
            u_t = u_eff_from_info
        else:
            c_t = None
