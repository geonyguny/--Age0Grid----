...
def _save_with_pdf(fig, path_png: str):
...
def plot_ew_vs_w(df_rep: pd.DataFrame, baselines: list[str], hjb_EW: float, OR: str, sub_hint: str, center_msg: str|None):
    ...
    if baselines:
        ...
        if len(d)>0: ax.plot(d["w_float"], d["EW_avg"], marker="o", label="rule"); lines_y.append(d["EW_avg"].values)
    ...
    _save_with_pdf(fig, os.path.join(OR, "fig_EW_vs_w_fixed.png")); plt.close(fig)
...
def plot_es_vs_w(df_rep: pd.DataFrame, baselines: list[str], hjb_ES: float, OR: str, sub_hint: str, center_msg: str|None):
    ...
    if baselines:
        ...
        if len(d)>0: ax.plot(d["w_float"], d["ES95_avg"], marker="o", label="rule"); lines_y.append(d["ES95_avg"].values)
    ...
    if is_empty or is_single:
        ...
        ax.legend(); fig.tight_layout()
        if center_msg: annotate_center(ax, center_msg, sub_hint)
    _save_with_pdf(fig, os.path.join(OR, "fig_ES95_vs_w_fixed.png")); plt.close(fig)
...
def plot_risk_return(df_rep: pd.DataFrame, baselines: list[str], hjb_EW: float, hjb_ES: float, OR: str, sub_hint: str, center_msg: str|None):
    ...
    for m in ["rule","hjb","rl"]:
        ...
        elif m=="hjb": ax.scatter(d["ES95_avg"], d["EW_avg"], label="HJB", **styles["hjb"])
        elif m=="rl":  ax.scatter(d["ES95_avg"], d["EW_avg"], label="RL",  **styles["rl"])
    ...
    if len(fr)==0 or not any_points:
        ...
        ax.legend(); fig.tight_layout()
        if center_msg: annotate_center(ax, center_msg, sub_hint)
    _save_with_pdf(fig, os.path.join(OR, "fig_risk_return.png")); plt.close(fig)
...
def plot_frontier(df_rep: pd.DataFrame, OR: str, sub_hint: str, center_msg: str|None):
    ...
    if len(fr) == 0 or not any_points:
        ...
        if center_msg: annotate_center(ax, center_msg, sub_hint)
    _save_with_pdf(fig, os.path.join(OR, "fig_frontier_EW_ES.png")); plt.close(fig)
...
def plot_ruin_bar(df_rep: pd.DataFrame, OR: str, sub_hint: str, center_msg: str | None):
    ...
    if d_rule.empty:
        ...
        _save_with_pdf(fig, os.path.join(OR, "fig_ruin_bar.png")); plt.close(fig); return
    w_order = ["w_0","w_0_3","w_0_5","w_0_7","w_1"]
    d_rule["w_fixed"] = pd.Categorical(d_rule["w_fixed"].astype(str), categories=w_order, ordered=True)
    baselines = [b for b in d_rule["baseline"].dropna().unique().tolist() if b != ""] or ["rule"]
    ...
    hatches = ["/","\\\\","xx","--","++","..","oo","**"]; edge_kwargs = dict(edgecolor="0.25", linewidth=0.6)
    bar_groups = []
    ...
    ncol = min(len(cols),4); leg = ax.legend(loc="upper center", bbox_to_anchor=(0.5,1.12), ncol=ncol, frameon=False)
    for t in leg.get_texts(): t.set_fontsize(9)
    fig.tight_layout(); 
    if center_msg: annotate_center(ax, center_msg, sub_hint)
    _save_with_pdf(fig, os.path.join(OR, "fig_ruin_bar.png")); plt.close(fig)
...
# DEV_2D_us0.6_h0.5 등에서 us, h 추출 → mix_us / hedge_sigma_k 보강 + la 추출
...
# === PATCH: alias-aware ensure_var_column ===
def ensure_var_column(df: pd.DataFrame, var_name: str, round_n: int | None = 3) -> pd.DataFrame:
    """
    Ensure `var_name` exists in df.
    Supports alias mapping: loss_aversion <- [bias_loss_aversion, la_k, la] and vice versa.
    If not found, tries to parse from tag with VAR_FROM_TAG_PATTERNS.
    """
    d = df.copy()
...
def make_oat_table(src: str, out: str, var: str, metrics: str, tag_startswith: str, method: str, es_mode: str, agg: str, round_var:int=3):
    ...
    # 기준점(첫 행) 대비 Δ% 추가 (median/mean일 때만)
...
def plot_heat(piv: pd.DataFrame, x: str, y: str, val: str, title: str,
    ...
    data = np.array(piv.values, dtype=float); mask = np.isnan(data)
    mdata = np.ma.masked_array(data, mask=mask)
    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
    cmap = getattr(plt.cm, cmap_name, plt.cm.viridis).copy(); cmap.set_bad("lightgray")
...
def _load_overlay_points(path: str) -> tuple[List[dict], Dict[str,str]]:
    ...
    try:
        ...
        print(f"[WARN] overlay_points load failed: {e}")
    return pts, meta
...
def make_oat_heatmaps(src: str, outdir: str, tag_startswith: str, x: str, y: str,
    ...
    if infer_from_tag: df = _infer_from_tag(df)
    ...
    # 수치 캐스팅
    for c in [x,y,"EW","ES95","CompositeScore","Ruin","hedge_ratio","hedge_sigma_k","mix_us","mix_kr","mix_gold",
              "loss_aversion","bias_loss_aversion"]:
        if c in df.columns: df[c] = pd.to_numeric(df[c], errors="coerce")

    # x/y가 없으면 tag/alias에서 보강
    ...
    if round_x is not None and x in df.columns: df[x] = df[x].round(round_x)
    if round_y is not None and y in df.columns: df[y] = df[y].round(round_y)
    ...
    print(f"[INFO] rows={len(df)} | x={x} y={y} | zlist={zcols} | agg={agg} | method={method or '-'} | es_mode={es_mode or '-'}")
    for z in zcols:
        if z not in df.columns:
            print(f"[SKIP] '{z}' 없음"); continue
        sub = df[[x,y,z]].dropna()
        if sub.empty:
            print(f"[SKIP] '{z}' 데이터 없음"); continue
        ...
        _save_with_pdf(fig, str(outpng)); plt.close(fig)
        if do_overlay:
            print(f"[OK] overlay -> {outpng}")
    ...
    if args.cmd == "figs":
        ...
        if args.paper_style == "on":
            try:
                ...
                print("[INFO] paper_style applied; PDF export enabled; overlay disabled.")
            except Exception as e:
                print(f"[WARN] paper_style failed: {e}"); SAVE_PDF = False; center_msg_forced_none = False
        else:
